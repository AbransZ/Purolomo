...
const db = new sql.Database(dbPath, (err) => {
    if (err) {
        ...
        console.log('✅ Conexión a la base de datos establecida correctamente:', dbPath);
        ...
        db.run(`CREATE TABLE IF NOT EXISTS aplicacion (
            ...
            if (errApp) {
                ...
                db.run(`CREATE TABLE IF NOT EXISTS registro_version (
                    id INTEGER PRIMARY KEY AUTOINCREMENT, 
                    version TEXT, 
                    descripcion TEXT, 
                    hash_git TEXT, 
                    ...
                    if (errReg) {
                        console.error('❌ Error tabla registro_version:', errReg.message);
                    } else {
...
app.post('/api/registros', (req, res) => {
        ...
        if (appRow) {
            ...
            db.run(insertAppSql, [nombre, autor], function (errInsertApp) {
                if (errInsertApp) {
                    ...
                    return res.status(500).json({ error: "Error interno al crear aplicación." });
                }
    ...
    function insertRegistro(id_app) {
        ...
        db.run(insertRegSql, params, function (errInsertReg) {
            ...
            console.log(`✅ Nuevo registro insertado con ID: ${registroId} para App ID: ${id_app}`);
